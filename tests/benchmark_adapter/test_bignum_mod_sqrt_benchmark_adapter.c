/**
 * @file test_bignum_mod_sqrt_benchmark_adapter.c
 * @brief Deterministic unit tests for the bignum_mod_sqrt benchmark-core adapter.
 *
 * @details
...
} bignum_mod_sqrt_adapter_test_status_t;
 ...
 * @return A named bignum_mod_sqrt_adapter_test_status_t result.
 *
 * @details
 ...
 */
static bignum_mod_sqrt_adapter_test_status_t bignum_mod_sqrt_adapter_test_make_workload(
    benchmark_workload_t *workload)
{
    ...
    *workload = (benchmark_workload_t){
    ...
    return BIGNUM_MOD_SQRT_ADAPTER_TEST_STATUS_SUCCESS;
}
 ...
 */
static bignum_mod_sqrt_adapter_test_status_t bignum_mod_sqrt_adapter_test_validation(void)
{
    benchmark_workload_t workload;
    benchmark_workload_t invalid_workload;

    if (bignum_mod_sqrt_adapter_test_make_workload(&workload) !=
        BIGNUM_MOD_SQRT_ADAPTER_TEST_STATUS_SUCCESS) {
        return BIGNUM_MOD_SQRT_ADAPTER_TEST_STATUS_FAILURE;
    }
    if (bignum_mod_sqrt_benchmark_validate_workload(&workload) !=
        BIGNUM_MOD_SQRT_BENCHMARK_STATUS_SUCCESS) {
        return BIGNUM_MOD_SQRT_ADAPTER_TEST_STATUS_FAILURE;
    }
    invalid_workload = workload;
    invalid_workload.operation_kind = "xor";
    if (bignum_mod_sqrt_benchmark_validate_workload(&invalid_workload) !=
        BIGNUM_MOD_SQRT_BENCHMARK_STATUS_INVALID_PROFILE) {
        return BIGNUM_MOD_SQRT_ADAPTER_TEST_STATUS_FAILURE;
    }
    if (bignum_mod_sqrt_benchmark_validate_workload(NULL) !=
        BIGNUM_MOD_SQRT_BENCHMARK_STATUS_NULL_ARGUMENT) {
        return BIGNUM_MOD_SQRT_ADAPTER_TEST_STATUS_FAILURE;
    }
    return BIGNUM_MOD_SQRT_ADAPTER_TEST_STATUS_SUCCESS;
}
 ...
 */
static bignum_mod_sqrt_adapter_test_status_t bignum_mod_sqrt_adapter_test_callbacks(void)
{
    ...
    return checksum == 0U
        ? BIGNUM_MOD_SQRT_ADAPTER_TEST_STATUS_FAILURE
        : BIGNUM_MOD_SQRT_ADAPTER_TEST_STATUS_SUCCESS;
}
...
int main(void)
{
    if (bignum_mod_sqrt_adapter_test_validation() !=
        BIGNUM_MOD_SQRT_ADAPTER_TEST_STATUS_SUCCESS) {
        fputs("bignum_mod_sqrt benchmark adapter validation test failed\n", stderr);
        return 1;
    }
    if (bignum_mod_sqrt_adapter_test_callbacks() !=
        BIGNUM_MOD_SQRT_ADAPTER_TEST_STATUS_SUCCESS) {
        fputs("bignum_mod_sqrt benchmark adapter callback test failed\n", stderr);
        return 1;
    }
    puts("bignum_mod_sqrt benchmark adapter tests: OK");
    return 0;
}
