 ...
 * @brief Deterministic bignum_mod_sqrt adapter implementation for benchmark-core.
 *
 * @details
...
#define BIGNUM_MOD_SQRT_BENCHMARK_FNV_OFFSET UINT64_C(1469598103934665603)
/** @brief Stable FNV-1a prime used for observable benchmark checksums. */
#define BIGNUM_MOD_SQRT_BENCHMARK_FNV_PRIME UINT64_C(1099511628211)
 ...
 */
typedef enum {
    BIGNUM_MOD_SQRT_SHIFT_ZERO = 0, /**< Always request a zero-bit shift. */
    BIGNUM_MOD_SQRT_SHIFT_BIT = 1, /**< Request a non-zero sub-word shift. */
    BIGNUM_MOD_SQRT_SHIFT_WORD = 2, /**< Request a whole-word shift when representable. */
    BIGNUM_MOD_SQRT_SHIFT_COMBINED = 3, /**< Request a word-plus-bit shift when representable. */
    BIGNUM_MOD_SQRT_SHIFT_RANDOM = 4, /**< Request a deterministic representable random shift. */
    BIGNUM_MOD_SQRT_SHIFT_MIXED = 5 /**< Alternate deterministic shift classes by iteration. */
} bignum_mod_sqrt_shift_kind_t;
 ...
 */
static bignum_mod_sqrt_benchmark_status_t bignum_mod_sqrt_benchmark_string_equals(
    const char *left,
    const char *right,
 ...
 */
static bignum_mod_sqrt_benchmark_status_t bignum_mod_sqrt_benchmark_value_is_allowed(
    const char *value,
    ...
    benchmark_boolean_t *accepted)
{
    ...
    *accepted = BENCHMARK_BOOLEAN_FALSE;
    for (size_t index = 0U; allowed[index] != NULL; ++index) {
        benchmark_boolean_t equal;
        bignum_mod_sqrt_benchmark_status_t status;

        status = bignum_mod_sqrt_benchmark_string_equals(value, allowed[index], &equal);
        if (status != BIGNUM_MOD_SQRT_BENCHMARK_STATUS_SUCCESS) {
            return status;
        }
 ...
 */
static bignum_mod_sqrt_benchmark_status_t bignum_mod_sqrt_benchmark_validate_axis(
    const char *value,
    const char *const *allowed)
{
    benchmark_boolean_t accepted;
    bignum_mod_sqrt_benchmark_status_t status =
        bignum_mod_sqrt_benchmark_value_is_allowed(value, allowed, &accepted);
    ...
    return accepted == BENCHMARK_BOOLEAN_TRUE
        ...
        : BIGNUM_MOD_SQRT_BENCHMARK_STATUS_INVALID_PROFILE;
}
 ...
 */
static bignum_mod_sqrt_benchmark_status_t bignum_mod_sqrt_benchmark_parse_shift_kind(
    const char *operation_kind,
    bignum_mod_sqrt_shift_kind_t *shift_kind)
{
    if (operation_kind == NULL || shift_kind == NULL) {
        return BIGNUM_MOD_SQRT_BENCHMARK_STATUS_NULL_ARGUMENT;
    }
    if (strcmp(operation_kind, "shift-zero") == 0) {
        *shift_kind = BIGNUM_MOD_SQRT_SHIFT_ZERO;
    } else if (strcmp(operation_kind, "shift-bit") == 0) {
        *shift_kind = BIGNUM_MOD_SQRT_SHIFT_BIT;
    } else if (strcmp(operation_kind, "shift-word") == 0) {
        *shift_kind = BIGNUM_MOD_SQRT_SHIFT_WORD;
    } else if (strcmp(operation_kind, "shift-combined") == 0) {
        *shift_kind = BIGNUM_MOD_SQRT_SHIFT_COMBINED;
    } else if (strcmp(operation_kind, "shift-random") == 0) {
        *shift_kind = BIGNUM_MOD_SQRT_SHIFT_RANDOM;
    } else if (strcmp(operation_kind, "shift-mixed") == 0) {
        *shift_kind = BIGNUM_MOD_SQRT_SHIFT_MIXED;
    } else {
 ...
 */
static bignum_mod_sqrt_benchmark_status_t bignum_mod_sqrt_benchmark_next_value(
    uint64_t *state,
    uint64_t *value)
{
 ...
 */
static bignum_mod_sqrt_benchmark_status_t bignum_mod_sqrt_benchmark_leading_zero_bits(
    uint64_t value,
    size_t *count)
{
    if (count == NULL) {
 ...
 */
static bignum_mod_sqrt_benchmark_status_t bignum_mod_sqrt_benchmark_max_safe_shift(
    ...
    size_t *maximum)
{
    ...
    status = bignum_mod_sqrt_benchmark_leading_zero_bits(
 ...
 */
static bignum_mod_sqrt_benchmark_status_t bignum_mod_sqrt_benchmark_choose_length(
    ...
    size_t *length)
{
    ...
    if (strcmp(workload->size_profile, "one") == 0) {
        ...
        *length = BIGNUM_CAPACITY / 2U == 0U ? 1U : BIGNUM_CAPACITY / 2U;
        ...
        status = bignum_mod_sqrt_benchmark_next_value(state, &random_value);
        if (status != BIGNUM_MOD_SQRT_BENCHMARK_STATUS_SUCCESS) {
            return status;
        }
 ...
 * scheduling.
 */
static bignum_mod_sqrt_benchmark_status_t bignum_mod_sqrt_benchmark_row_is_zero(
 ...
 */
static benchmark_adapter_status_t bignum_mod_sqrt_benchmark_initialize(
    void *state,
    uint64_t sequence_index,
    const benchmark_workload_t *workload,
    void *adapter_context)
{
    ...
    status = bignum_mod_sqrt_benchmark_validate_workload(workload);
    if (status != BIGNUM_MOD_SQRT_BENCHMARK_STATUS_SUCCESS) {
        return BENCHMARK_ADAPTER_STATUS_INPUT_ERROR;
    }
    memset(number, 0, sizeof(*number));
    status = bignum_mod_sqrt_benchmark_row_is_zero(workload->input_kind, sequence_index, &zero);
    if (status != BIGNUM_MOD_SQRT_BENCHMARK_STATUS_SUCCESS) {
        return BENCHMARK_ADAPTER_STATUS_INPUT_ERROR;
    }
    ...
    status = bignum_mod_sqrt_benchmark_choose_length(workload, &random_state, &length);
    if (status != BIGNUM_MOD_SQRT_BENCHMARK_STATUS_SUCCESS || length == 0U || length > BIGNUM_CAPACITY) {
        return BENCHMARK_ADAPTER_STATUS_INPUT_ERROR;
    }
    number->len = length;
    for (size_t word = 0U; word < length; ++word) {
        status = bignum_mod_sqrt_benchmark_next_value(&random_state, &random_value);
        if (status != BIGNUM_MOD_SQRT_BENCHMARK_STATUS_SUCCESS) {
            return BENCHMARK_ADAPTER_STATUS_INPUT_ERROR;
        }
 ...
 */
static bignum_mod_sqrt_benchmark_status_t bignum_mod_sqrt_benchmark_choose_shift(
    const bignum_t *number,
    bignum_mod_sqrt_shift_kind_t shift_kind,
    uint64_t iteration,
    uint64_t seed,
    size_t *shift_amount)
{
    ...
    status = bignum_mod_sqrt_benchmark_max_safe_shift(number, &maximum);
    if (status != BIGNUM_MOD_SQRT_BENCHMARK_STATUS_SUCCESS) {
        return status;
    }
    if (number->len == 0U || shift_kind == BIGNUM_MOD_SQRT_SHIFT_ZERO || maximum == 0U) {
        *shift_amount = 0U;
        return BIGNUM_MOD_SQRT_BENCHMARK_STATUS_SUCCESS;
    }
    if (shift_kind == BIGNUM_MOD_SQRT_SHIFT_MIXED) {
        shift_kind = (bignum_mod_sqrt_shift_kind_t)(iteration % UINT64_C(4));
    }
    status = bignum_mod_sqrt_benchmark_next_value(&random_state, &random_value);
    if (status != BIGNUM_MOD_SQRT_BENCHMARK_STATUS_SUCCESS) {
        return status;
    }
    if (shift_kind == BIGNUM_MOD_SQRT_SHIFT_BIT) {
        *shift_amount = 1U + (size_t)(random_value % (maximum < 63U ? maximum : 63U));
    } else if (shift_kind == BIGNUM_MOD_SQRT_SHIFT_WORD) {
        *shift_amount = maximum / 64U == 0U ? 0U :
            64U * (1U + (size_t)(random_value % (maximum / 64U)));
    } else if (shift_kind == BIGNUM_MOD_SQRT_SHIFT_COMBINED) {
...
/**
 * @brief Execute bignum_mod_sqrt once on a benchmark-core mutable state copy.
 ...
 * amount, invokes bignum_mod_sqrt in-place, and maps its named library status
 * to benchmark-core's named callback result.
 */
static benchmark_adapter_status_t bignum_mod_sqrt_benchmark_operation(
    void *state,
    uint64_t iteration,
    const benchmark_workload_t *workload,
    void *adapter_context)
{
        bignum_t *number = state;
    bignum_t modulus = {.words = {11U}, .len = 1U};
    bignum_t root = {0};
    bignum_mod_sqrt_status_t operation_status;
    (void)iteration;
    (void)workload;
    (void)adapter_context;
    if (number == NULL || workload == NULL) {
    ...
    number->words[0] = 4U;
    number->len = 1U;
    operation_status = bignum_mod_sqrt(number, &modulus, &root);
    if (operation_status != BIGNUM_MOD_SQRT_SUCCESS) {
 ...
 */
static uint64_t bignum_mod_sqrt_benchmark_checksum(
    const void *state,
    uint64_t iteration,
    void *adapter_context)
{
    const bignum_t *number = state;
    uint64_t checksum = BIGNUM_MOD_SQRT_BENCHMARK_FNV_OFFSET;
    ...
    for (size_t word = 0U; word < BIGNUM_CAPACITY; ++word) {
        ...
        checksum *= BIGNUM_MOD_SQRT_BENCHMARK_FNV_PRIME;
    }
    ...
    const benchmark_workload_t *workload)
{
    ...
    status = bignum_mod_sqrt_benchmark_validate_axis(workload->input_kind, input_values);
    if (status != BIGNUM_MOD_SQRT_BENCHMARK_STATUS_SUCCESS) {
        return status;
    }
    status = bignum_mod_sqrt_benchmark_validate_axis(workload->operation_kind, operation_values);
    if (status != BIGNUM_MOD_SQRT_BENCHMARK_STATUS_SUCCESS) {
        return status;
    }
    status = bignum_mod_sqrt_benchmark_validate_axis(workload->measure_mode, measure_values);
    if (status != BIGNUM_MOD_SQRT_BENCHMARK_STATUS_SUCCESS) {
        return status;
    }
    status = bignum_mod_sqrt_benchmark_validate_axis(workload->size_profile, size_values);
    if (status != BIGNUM_MOD_SQRT_BENCHMARK_STATUS_SUCCESS) {
        return status;
    }
    return bignum_mod_sqrt_benchmark_validate_axis(workload->capacity_profile, capacity_values);
}
    ...
    benchmark_adapter_t *adapter)
{
    ...
    *adapter = (benchmark_adapter_t){
        ...
        .initialize = bignum_mod_sqrt_benchmark_initialize,
        .operation = bignum_mod_sqrt_benchmark_operation,
        .checksum = bignum_mod_sqrt_benchmark_checksum
    };
    return BIGNUM_MOD_SQRT_BENCHMARK_STATUS_SUCCESS;
}
